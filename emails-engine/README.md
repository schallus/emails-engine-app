...
email_engine
  src
    clients
      brands.json <-- Contient toutes les marques ainsi que des metadonnées sur ces marques
      clientA
      clientB
      clientC
        campaigns.json <-- Contient la liste de toutes les campagnes créé
        blocks.json <-- Liste des blocs disponibles avec leurs propriétés
        campaignA
        campaignB
        campaignC
          config.json <-- Configuration de la campagne (Sujet des emails et layout utilisé)
          data
            data-lang.json <-- Contient les traductions pour chaque bloc
          pages
            structure.json <-- Contient la structure de l'email
```
